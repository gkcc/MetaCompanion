...
foreach ($deckStatsFile in $existingDeckStatsPaths) {
	$doc = New-Object System.Xml.XmlDocument
	$doc.Load($deckStatsFile)
	...
	foreach ($game in @($doc.SelectNodes("//Game"))) {
		...
		$gameId = Get-NodeText -Node $game -Name "GameId"
		$gameKey = if ([string]::IsNullOrWhiteSpace($gameId)) {
			"{0}|{1}|{2}|{3}|{4}" -f $startTime.ToString("o"), (Get-NodeText -Node $game -Name "PlayerHero"), (Get-NodeText -Node $game -Name "OpponentHero"), (Get-NodeText -Node $game -Name "Result"), $mode
		} else {
			$gameId
		}
		if ($seenGames.ContainsKey($gameKey)) {
			continue
		}
